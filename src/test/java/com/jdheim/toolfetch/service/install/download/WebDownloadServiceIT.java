...
class WebDownloadServiceIT {
    ...
    void setUp() {
        ...
        testLogListAppender.start(WebDownloadService.class, ToolUriTransformer.class);
    }
    ...
    void testDownload_ForbiddenScheme(WireMockRuntimeInfo wmRuntimeInfo) {
        ...
        testLogListAppender.assertAnyMatch(Level.WARN, "Forbidden scheme detected. Only http/https are allowed");
        testLogListAppender.assertAnyMatch(Level.WARN, "URI could not be resolved. Skipping " + tool.id());
    }
    ...
    void testDownload_MissingVersion(WireMockRuntimeInfo wmRuntimeInfo) {
        ...
        assertThat(path).isEmpty();
        assertThat(tempDir.resolve(tool.id())).doesNotExist();
        testLogListAppender.assertAnyMatch(Level.WARN, "Missing required parameter: version");
        testLogListAppender.assertAnyMatch(Level.WARN, "URI could not be resolved. Skipping " + tool.id());
    }
    ...
    void testDownload_Non200Response(WireMockRuntimeInfo wmRuntimeInfo) {
        ...
        assertThat(path).isEmpty();
        assertThat(tempDir.resolve(tool.id())).doesNotExist();
        testLogListAppender.assertAnyMatch(Level.WARN, "Download failed: received HTTP 404 response. Skipping " + tool.id());
    }
    ...
    void testDownload_UnknownArchiveName(WireMockRuntimeInfo wmRuntimeInfo) {
        ...
        assertThat(path).isEmpty();
        assertThat(tempDir.resolve(tool.id())).doesNotExist();
        testLogListAppender.assertAnyMatch(Level.WARN, "Archive name could not be resolved. Skipping " + tool.id());
    }
